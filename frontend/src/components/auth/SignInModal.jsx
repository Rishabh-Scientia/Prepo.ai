...
export function SignInModal() {
  ...
  if (!isAuthModalOpen || authModalMode !== 'signin') return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white rounded-card border border-surface-200 shadow-elevated w-full max-w-md overflow-hidden animate-scaleUp">
        ...
        <div className="p-5 border-b border-surface-100 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-bold text-gray-900">Welcome back to Prepo.ai</h2>
            <p className="text-xs text-gray-500 mt-0.5">Sign in to generate quizzes and view history</p>
          </div>
          <button 
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 px-4 bg-primary-600 hover:bg-primary-700 text-white font-semibold text-sm rounded-card transition shadow-sm flex items-center justify-center gap-2"
            >

...
def validate_doc_quiz_json(state: GenerateDocState) -> GenerateDocState:
    ...
    for i, q in enumerate(questions):
        ...
        question_text = str(q.get("question", "")).replace("\x0c", r"\f").replace("\x08", r"\b").strip()
        question_text = re.sub(r"(?<!\\)int_", r"\\int_", question_text)
        question_text = re.sub(r"(?<!\\)sum_", r"\\sum_", question_text)
        question_text = re.sub(r"(?<!\\)lim_", r"\\lim_", question_text)
        question_text = re.sub(r"(?<!\\)frac\{", r"\\frac{", question_text)
        question_text = re.sub(r"(?<!\\)sqrt\{", r"\\sqrt{", question_text)
        ...
        corr = str(correct_answer).replace("\x0c", r"\f").replace("\x08", r"\b")
